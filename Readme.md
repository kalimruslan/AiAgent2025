...
## 🤖 MCP Server - Интеграция с Claude Desktop

Этот проект включает полнофункциональный **MCP (Model Context Protocol) сервер**, который можно подключить к Claude Desktop и другим MCP-совместимым клиентам.

### Быстрый старт

1. **Настройте API ключи** в `local.properties`:
   ```properties
...
2. **Протестируйте сервер**:
   ```bash
   ./gradlew :server:runStdio
   ```

3. **Настройте Claude Desktop** - добавьте в конфигурацию:
   ```json
   {
     "mcpServers": {
       "llm-agent": {
         "command": "/путь/к/проекту/server/run-mcp-server.sh",
         "env": {
           "TRELLO_API_KEY": "ваш_ключ",
           "TRELLO_TOKEN": "ваш_токен",
           "OPENWEATHER_API_KEY": "ваш_ключ"
...
**Погода:**
- `getWeather` - получение текущей погоды для города

**Trello - Базовые операции:**
- `trello_getCards` - список карточек с доски
- `trello_createCard` - создание новой карточки
- `trello_getCard` - информация о карточке
- `trello_getSummary` - статистика по доске

**Trello - Умные операции:**
- `trello_quickTask` - быстрое создание задачи с умным парсингом дедлайна
- `trello_moveCard` - перемещение карточки между списками
- `trello_updateCard` - обновление карточки
- `trello_searchCards` - поиск карточек по критериям

**Trello - Массовые операции:**
- `trello_batchCreateCards` - массовое создание карточек
- `trello_bulkUpdate` - массовое обновление
- `trello_bulkMove` - массовое перемещение

### Документация

- 📖 [Быстрый старт](server/QUICKSTART.md) - подключение за 3 шага
- 📚 [Полная документация](server/README_CLAUDE_INTEGRATION.md) - детальное руководство
- 🔧 [Конфигурация](server/claude-desktop-config.json) - пример конфигурации

### Архитектура

```
server/
├── StdioServer.kt          # Stdio транспорт для Claude Desktop
├── KtorServer.kt           # HTTP транспорт для вашего приложения
├── McpServerSdk.kt         # Логика MCP сервера
├── SdkJsonRpcHandler.kt    # JSON-RPC обработчик
└── api/
    ├── TrelloClient.kt     # Интеграция с Trello API
    └── OpenWeatherMapClient.kt  # Интеграция с OpenWeatherMap API
```

### Примеры использования

После подключения к Claude Desktop, вы можете использовать естественный язык:
...
```
Покажи статистику моей доски Trello (boardId: YOUR_BOARD_ID)
```

```
Найди все просроченные задачи на доске (boardId: YOUR_BOARD_ID)
```
