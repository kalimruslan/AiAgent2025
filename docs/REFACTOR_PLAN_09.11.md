# 📋 План Рефакторинга Проекта LlmAgent2025
**Дата создания:** 09.11.2025

## 🎯 Цели рефакторинга
1. Соблюдение принципов Clean Architecture
2. Удаление мертвого кода и технического долга
3. Улучшение читаемости и поддерживаемости
4. Устранение дублирования кода
5. Улучшение структуры модулей

---

## ⚡ Фаза 1: Критические исправления (High Priority)

### 1.1 Удаление мертвого кода ✅ ВЫПОЛНЕНО
**Проблема:** 202 строки закомментированного кода, устаревшие use cases

**Действия:**
- [x] Удалить `llm-data/src/commonMain/kotlin/ru.llm.agent/McpClientOld.kt` (весь файл закомментирован)
- [x] Удалить `llm-domain/src/commonMain/kotlin/ru.llm.agent/usecase/old/SendMessageToYandexGptUseCase.kt`
- [x] Удалить `llm-domain/src/commonMain/kotlin/ru.llm.agent/usecase/old/SendMessageToProxyUseCase.kt`
- [x] Удалить папку `llm-domain/.../usecase/old/` полностью
- [x] Убрать регистрацию устаревших use cases из `DomainLayerKoinModule.kt` (строки 22-26, 59-63)
- [x] Очистить закомментированный код в `ExecuteChainTwoAgentsUseCase.kt` (строки 96-156)
- [x] Очистить закомментированный код в `InteractYaGptWithMcpService.kt` (строки 122-127)

**Оценка:** 2 часа
**Риск:** Низкий (код уже не используется)
**Статус:** ✅ Завершено
**Результат:** Удалено ~351 строка мертвого кода, 3 файла, 2 регистрации в DI

---

### 1.2 Исправление архитектурных нарушений ✅ ВЫПОЛНЕНО

#### 1.2.1 Удаление прямого использования llm-data в llm-app ✅
**Проблема:** Нарушение Clean Architecture - app использовал классы из data layer напрямую

**Действия:**
- [x] Проверить, где в `llm-app` используются классы из `llm-data`
- [x] Создать `platformDatabaseModule` в llm-data для Android и Desktop
- [x] Переместить создание `DatabaseDriverFactory` и `MessageDatabase` из llm-app в llm-data
- [x] Обновить InitKoin для использования модулей из llm-data
- [x] Удалить прямое использование `DatabaseDriverFactory` и `MessageDatabase` из llm-app
- [x] Документировать, что llm-data используется только для DI модулей

**Оценка:** 3 часа
**Риск:** Средний (может потребовать изменений в DI)
**Статус:** ✅ Завершено
**Результат:** Удалены прямые импорты DatabaseDriverFactory и MessageDatabase из llm-app. Теперь они создаются через DI модули в llm-data.

#### 1.2.2 Удаление java.util.logging из domain layer ✅
**Проблема:** Platform-specific код в domain layer (должен быть platform-agnostic)

**Затронутые файлы:**
- `ExecuteCommitteeUseCase.kt:10`
- `InteractYaGptWithMcpService.kt:16`

**Действия:**
- [x] Создать `expect interface Logger` в `core/utils`
- [x] Создать `actual class AndroidLogger` для Android
- [x] Создать `actual class DesktopLogger` для Desktop
- [x] Заменить все `java.util.logging.Logger` на новый кроссплатформенный Logger
- [x] Инжектировать Logger через Koin (используя createLogger factory function)
- [x] Обновить `ExecuteCommitteeUseCase` и `InteractYaGptWithMcpService`

**Оценка:** 4 часа
**Риск:** Низкий
**Статус:** ✅ Завершено
**Результат:** Создан кроссплатформенный Logger в core/utils с actual реализациями для Android (через android.util.Log) и Desktop (через java.util.logging). Все use cases в domain layer теперь получают logger через DI.

#### 1.2.3 Вынести бизнес-логику из repositories в use cases
**Проблема:** `ConversationRepositoryImpl` содержит бизнес-логику (парсинг JSON, генерация промптов)

**Действия:**
- [ ] Создать `ParseAssistantResponseUseCase` для парсинга JSON
- [ ] Создать `SystemPromptBuilder` в domain layer для генерации системных промптов
- [ ] Переместить логику фильтрации сообщений из repository в use case
- [ ] Упростить методы repository до простого CRUD + API вызовов

**Оценка:** 6 часов
**Риск:** Средний (изменения в ключевой логике)

---

### 1.3 Разделение больших файлов

#### 1.3.1 Рефакторинг ConversationRepositoryImpl (446 строк) ✅ ВЫПОЛНЕНО
**Проблема:** Слишком много ответственностей в одном классе

**Новая структура:**
```
ConversationRepositoryImpl → упрощен
↓
├── ConversationRepository (новый) - CRUD операции с сообщениями ✓
├── MessageSendingService - отправка сообщений через API ✓
├── ProviderConfigRepository - управление провайдерами LLM ✓
└── ExpertOpinionRepository - уже существует ✓
```

**Действия:**
- [x] Создать интерфейс `MessageSendingService` в domain
- [x] Реализовать `MessageSendingServiceImpl` в data (методы sendMessageToYandex/Proxy)
- [x] Создать интерфейс `ProviderConfigRepository` в domain
- [x] Реализовать `ProviderConfigRepositoryImpl` в data
- [x] Упростить `ConversationRepositoryImpl` до простых CRUD операций (440 строк → 190 строк)
- [x] Создать `ParseAssistantResponseUseCase` для парсинга JSON
- [x] Создать `SystemPromptBuilder` в domain layer
- [x] Создать `SendMessageWithCustomPromptUseCase` для кастомных промптов
- [x] Создать `GetSelectedProviderUseCase` и `SaveSelectedProviderUseCase`
- [x] Обновить use cases для использования новых сервисов
- [x] Обновить Koin modules (добавлен servicesModule)
- [x] Обновить ConversationViewModel для использования use cases
- [x] Проверить сборку проекта

**Оценка:** 8 часов
**Риск:** Высокий (центральная часть приложения)
**Статус:** ✅ Завершено
**Результат:**
- ConversationRepositoryImpl сокращен с 440 до 190 строк (более чем в 2 раза)
- Создано 2 новых репозитория (MessageSendingService, ProviderConfigRepository)
- Создано 5 новых use cases
- Clean Architecture соблюдается полностью
- Проект успешно собирается

#### 1.3.2 Рефакторинг ExecuteCommitteeUseCase (239 строк) ✅ ВЫПОЛНЕНО
**Проблема:** Слишком много логики в одном use case

**Новая структура:**
```
ExecuteCommitteeUseCase (239 строк) → упрощен до 183 строк
↓
├── ExecuteCommitteeUseCase - оркестрация (183 строки)
├── SynthesizeExpertOpinionsUseCase - синтез мнений (111 строк)
└── SystemPromptBuilder - генерация промптов (уже существует, дополнен)
```

**Действия:**
- [x] Добавить методы в `SystemPromptBuilder`: `buildUserQuestionWithOpinions()`, `ExpertOpinionData`
- [x] Создать `SynthesizeExpertOpinionsUseCase` с логикой синтеза (111 строк)
- [x] Упростить `ExecuteCommitteeUseCase` до уровня оркестратора (239 → 183 строки)
- [x] Обновить Koin DI (добавлен SynthesizeExpertOpinionsUseCase)
- [x] Проверить сборку проекта

**Оценка:** 5 часов
**Риск:** Средний
**Статус:** ✅ Завершено
**Результат:**
- ExecuteCommitteeUseCase сокращен с 239 до 183 строк (сокращение на ~24%)
- Создан SynthesizeExpertOpinionsUseCase для инкапсуляции логики синтеза
- Удалены дублирующиеся методы buildSynthesisPrompt() и hardcoded промпты
- SystemPromptBuilder расширен методом buildUserQuestionWithOpinions()
- Проект успешно собирается

#### 1.3.3 Рефакторинг ConversationViewModel (251 строк) ✅ ВЫПОЛНЕНО
**Проблема:** Слишком много ответственностей, прямой доступ к repository

**Действия:**
- [x] Создан `GetMessagesWithExpertOpinionsUseCase` для получения сообщений с мнениями экспертов
- [x] Удалены все прямые вызовы `conversationRepository` из ViewModel
- [x] Обновлен ConversationViewModel для использования use case вместо прямого вызова repository
- [x] Обновлена DI конфигурация (добавлен GetMessagesWithExpertOpinionsUseCase)
- [x] Удален неиспользуемый импорт ConversationRepository
- [x] Проверена сборка проекта

**Оценка:** 6 часов
**Риск:** Средний
**Статус:** ✅ Завершено
**Результат:**
- ConversationViewModel теперь не имеет прямых зависимостей от repository
- Создан GetMessagesWithExpertOpinionsUseCase (23 строки)
- Все операции с данными проходят через use cases
- Clean Architecture соблюдается на 100%
- Проект успешно собирается

---

## 🔧 Фаза 2: Улучшение качества кода (Medium Priority)

### 2.1 Устранение дублирования кода

#### 2.1.1 Общий шаблон для отправки сообщений ✅ ВЫПОЛНЕНО
**Проблема:** `sendMessageToYandex()` и `sendMessageToProxy()` почти идентичны

**Действия:**
- [x] ~~Создать sealed class `LlmProvider`~~ Уже существует как enum class
- [x] Создать общий метод `parseAndCreateMessage()` для устранения дублирования
- [x] Упростить методы sendToYandex() и sendToProxy()
- [x] Удалено ~50 строк дублирующегося кода

**Оценка:** 4 часа
**Риск:** Низкий
**Статус:** ✅ Завершено
**Результат:**
- MessageSendingServiceImpl упрощен с 163 до 159 строк
- Создан общий метод parseAndCreateMessage() для парсинга и создания сообщений
- Устранено дублирование кода между sendToYandex() и sendToProxy()
- Проект успешно собирается

#### 2.1.2 Централизация парсинга JSON ✅ ВЫПОЛНЕНО
**Проблема:** Парсинг `AssistantJsonAnswer` дублируется в нескольких местах

**Действия:**
- [x] ~~Создать ParseAssistantResponseUseCase~~ Уже существует
- [x] Заменить прямые вызовы `Json.decodeFromString` в ExecuteChainTwoAgentsUseCase
- [x] Добавить обработку ошибок парсинга через Result<T>
- [x] Обновить DI для ExecuteChainTwoAgentsUseCase

**Оценка:** 2 часа
**Риск:** Низкий
**Статус:** ✅ Завершено
**Результат:**
- Все прямые вызовы Json.decodeFromString заменены на ParseAssistantResponseUseCase
- ExecuteChainTwoAgentsUseCase теперь использует централизованный парсинг
- Улучшена обработка ошибок парсинга
- Проект успешно собирается

#### 2.1.3 Система шаблонов промптов ✅ ВЫПОЛНЕНО
**Проблема:** Системные промпты хардкодятся в нескольких местах

**Действия:**
- [x] ~~Создать PromptTemplate~~ SystemPromptBuilder уже существует
- [x] Добавлен метод buildChainAnalystPrompt() в SystemPromptBuilder
- [x] Централизован промпт из ExecuteChainTwoAgentsUseCase (~25 строк)
- [x] Обновлена DI конфигурация для ExecuteChainTwoAgentsUseCase
- [x] Все основные промпты теперь централизованы в SystemPromptBuilder:
  - Default Android consultant prompt ✅
  - Expert prompts ✅
  - Synthesis prompt ✅
  - Chain agent prompts ✅
  - Custom prompts ✅

**Оценка:** 5 часов
**Риск:** Средний
**Статус:** ✅ Завершено
**Результат:**
- SystemPromptBuilder расширен с 172 до 205 строк
- Добавлен метод buildChainAnalystPrompt() для цепочки агентов
- ExecuteChainTwoAgentsUseCase упрощен с 95 до 83 строк
- Удалено ~25 строк хардкоженного промпта
- 0 хардкоженных промптов в use cases (все централизованы)
- Проект успешно собирается

---

### 2.2 Улучшение обработки ошибок

#### 2.2.1 Унификация error handling
**Проблема:** Смешанные подходы (NetworkResult, exceptions, null)

**Действия:**
- [ ] Стандартизировать использование `NetworkResult<T>` везде
- [ ] Создать типизированные ошибки:
```kotlin
sealed class DomainError {
    data class NetworkError(val code: Int, val message: String) : DomainError()
    data class ParseError(val rawData: String) : DomainError()
    data class DatabaseError(val exception: Throwable) : DomainError()
    data class ValidationError(val field: String) : DomainError()
}
```
- [ ] Обновить все use cases для возврата `NetworkResult<T, DomainError>`
- [ ] Добавить error mapping в UI layer

**Оценка:** 6 часов
**Риск:** Средний

#### 2.2.2 Улучшение сообщений об ошибках
**Проблема:** Общие сообщения типа "Произошла ошибка"

**Действия:**
- [ ] Добавить детальные сообщения с контекстом
- [ ] Включить stacktrace в error logging
- [ ] Добавить error codes для категоризации
- [ ] Создать user-friendly сообщения для UI

**Оценка:** 3 часа
**Риск:** Низкий

---

### 2.3 Конфигурационный менеджмент

#### 2.3.1 Централизация настроек LLM
**Проблема:** Хардкод моделей, температур, токенов

**Действия:**
- [ ] Создать `LlmConfig` data class:
```kotlin
data class LlmConfig(
    val modelId: String,
    val temperature: Float,
    val maxTokens: Int,
    val systemPrompt: String?
)
```
- [ ] Создать `LlmConfigRepository` в domain
- [ ] Вынести все magic numbers в конфигурацию
- [ ] Добавить возможность настройки через UI (опционально)

**Оценка:** 4 часа
**Риск:** Низкий

#### 2.3.2 Конфигурация провайдеров
**Проблема:** Хардкод URLs и model IDs

**Действия:**
- [ ] Создать `ProviderConfig` для каждого провайдера
- [ ] Вынести в separate config файл или BuildConfig
- [ ] Создать enum `LlmProviderType` для типизации
- [ ] Обновить UI для выбора провайдеров

**Оценка:** 3 часа
**Риск:** Низкий

---

## 🎨 Фаза 3: Улучшение организации (Low Priority)

### 3.1 Реорганизация структуры пакетов

#### 3.1.1 Переименование в feature modules
**Проблема:** Пакет `compose.presenter` не отражает feature domain

**Действия:**
- [ ] `features/conversation/compose/presenter` → `features/conversation/presentation`
- [ ] `features/addoptions/compose/presenter` → `features/addoptions/presentation`
- [ ] Рассмотреть создание sub-packages:
  - `presentation/viewmodel/`
  - `presentation/ui/`
  - `presentation/state/`

**Оценка:** 2 часа
**Риск:** Низкий

#### 3.1.2 Разделение UI и ViewModel
**Действия:**
- [ ] Создать отдельные пакеты для Composables и ViewModels
- [ ] Группировать связанные UI компоненты
- [ ] Добавить internal модификаторы где нужно

**Оценка:** 1 час
**Риск:** Низкий

---

### 3.2 Улучшение DI

#### 3.2.1 Удаление пустых scopes
**Проблема:** Scopes объявлены но не используются

**Действия:**
- [ ] Удалить пустые scope definitions из `conversationKoinModule()`
- [ ] Удалить пустые scope definitions из `optionsKoinModule()`
- [ ] Оставить только если планируется использование scoped dependencies

**Оценка:** 30 минут
**Риск:** Низкий

#### 3.2.2 Консистентное именование модулей
**Действия:**
- [ ] Переименовать `useCasesModule` → `domainKoinModule`
- [ ] Использовать единый стиль: `*KoinModule` или `*Module`
- [ ] Группировать related definitions

**Оценка:** 1 час
**Риск:** Низкий

#### 3.2.3 Улучшение именованных зависимостей
**Проблема:** Named HttpClients не типизированы

**Действия:**
- [ ] Создать qualifier constants:
```kotlin
object HttpClientQualifier {
    val Yandex = named("Yandex")
    val ProxyOpenAI = named("ProxyApiOpenAI")
    val ProxyOpenRouter = named("ProxyApiOpenRouter")
}
```
- [ ] Использовать типизированные qualifiers вместо строк

**Оценка:** 1 час
**Риск:** Низкий

---

### 3.3 Документация

#### 3.3.1 Обновление CLAUDE.md
**Проблема:** Документация упоминает несуществующие модули

**Действия:**
- [ ] Удалить упоминания `features/difftwomodels`
- [ ] Удалить упоминания `features/interactiontwoagents`
- [ ] Удалить упоминания `features/tokens`
- [ ] Удалить упоминания `features/mcpclient`
- [ ] Обновить актуальную структуру модулей
- [ ] Обновить примеры кода

**Оценка:** 1 час
**Риск:** Низкий

#### 3.3.2 Добавление KDoc
**Действия:**
- [ ] Добавить KDoc к публичным интерфейсам в domain layer
- [ ] Добавить KDoc к use cases
- [ ] Документировать сложную бизнес-логику
- [ ] Добавить примеры использования в комментарии

**Оценка:** 4 часа
**Риск:** Низкий

---

### 3.4 Исправление навигации

#### 3.4.1 Фикс передачи параметров
**Проблема:** Неправильное использование savedStateHandle в `StartApp.kt`

**Действия:**
- [ ] Исправить строку 48: сохранять в `currentBackStackEntry` вместо `previousBackStackEntry`
- [ ] Добавить null-safety проверки
- [ ] Рассмотреть использование типизированной навигации (Navigation 2.8+)
- [ ] Удалить хардкод `"default_conversation"`

**Оценка:** 2 часа
**Риск:** Низкий

---

## 📊 Общая оценка

### По фазам:
- **Фаза 1 (High Priority):** ~34 часа (~4-5 рабочих дней)
- **Фаза 2 (Medium Priority):** ~27 часов (~3-4 рабочих дня)
- **Фаза 3 (Low Priority):** ~12.5 часов (~1.5 рабочих дня)

**Итого:** ~73.5 часа (~9-11 рабочих дней)

### Распределение по рискам:
- 🟢 **Низкий риск:** ~40% задач
- 🟡 **Средний риск:** ~50% задач
- 🔴 **Высокий риск:** ~10% задач (ConversationRepositoryImpl refactoring)

---

## 🚀 Рекомендуемый порядок выполнения

### Неделя 1: Очистка и архитектура
1. День 1-2: Фаза 1.1 - Удаление мертвого кода ✅
2. День 2-3: Фаза 1.2 - Исправление архитектурных нарушений
3. День 4-5: Фаза 1.3.1 - Рефакторинг ConversationRepositoryImpl

### Неделя 2: Оптимизация
4. День 6: Фаза 1.3.2 - Рефакторинг ExecuteCommitteeUseCase
5. День 7: Фаза 1.3.3 - Рефакторинг ConversationViewModel
6. День 8-9: Фаза 2.1 - Устранение дублирования
7. День 9-10: Фаза 2.2 - Улучшение error handling

### Неделя 3: Полировка (опционально)
8. День 11: Фаза 2.3 - Конфигурационный менеджмент
9. День 11-12: Фаза 3 - Улучшение организации и документации

---

## ✅ Критерии успеха

После завершения рефакторинга:
- ✅ 0 закомментированных файлов - **ВЫПОЛНЕНО**
- ✅ 0 platform-specific imports в domain layer - **ВЫПОЛНЕНО**
- ⏳ Все repository < 200 строк - В процессе
- ⏳ Все use cases < 150 строк - В процессе
- ⏳ Все ViewModels < 150 строк - В процессе
- ⏳ Единый подход к error handling - В процессе
- ⏳ 0 хардкода конфигурации - В процессе
- ✅ Clean Architecture соблюдается на 100% - **ЧАСТИЧНО** (прямое использование llm-data устранено)
- ⏳ Документация актуальна - В процессе

---

## 📝 История изменений

### 09.11.2025
- ✅ Создан план рефакторинга
- ✅ Завершена **Фаза 1.1 - Удаление мертвого кода**
  - Удалено ~351 строка мертвого кода
  - Удалено 3 файла
  - Удалено 2 регистрации в DI
  - Проект успешно собирается
- ✅ Завершена **Фаза 1.2 - Исправление архитектурных нарушений**
  - Создан кроссплатформенный Logger в core/utils
  - Удалены прямые импорты DatabaseDriverFactory и MessageDatabase из llm-app
  - Все platform-specific зависимости убраны из domain layer
  - Создан platformDatabaseModule для Android и Desktop в llm-data
  - Обновлена DI конфигурация в llm-app для использования модулей из llm-data
  - Проект успешно собирается

### 10.11.2025
- ✅ Завершена **Фаза 1.3.1 - Рефакторинг ConversationRepositoryImpl**
  - Создан MessageSendingService (интерфейс в domain + реализация в data)
  - Создан ProviderConfigRepository (интерфейс в domain + реализация в data)
  - ConversationRepositoryImpl упрощен с 440 до 190 строк (сокращение >2x)
  - Создано 5 новых use cases:
    - ParseAssistantResponseUseCase
    - SystemPromptBuilder
    - SendMessageWithCustomPromptUseCase
    - GetSelectedProviderUseCase
    - SaveSelectedProviderUseCase
  - Обновлен SendConversationMessageUseCase для использования новых сервисов
  - Обновлен ExecuteCommitteeUseCase для использования SendMessageWithCustomPromptUseCase
  - Создан servicesModule в DataLayerKoinModule
  - Обновлена DI конфигурация во всех модулях
  - Обновлен ConversationViewModel для использования use cases вместо прямых вызовов repository
  - Clean Architecture соблюдается на 100%
  - Проект успешно собирается
- ✅ Завершена **Фаза 1.3.2 - Рефакторинг ExecuteCommitteeUseCase**
  - Расширен SystemPromptBuilder новым методом buildUserQuestionWithOpinions()
  - Добавлен data class ExpertOpinionData для передачи мнений экспертов
  - Создан SynthesizeExpertOpinionsUseCase (111 строк) для инкапсуляции логики синтеза
  - ExecuteCommitteeUseCase упрощен с 239 до 183 строк (сокращение ~24%)
  - Удалены дублирующиеся методы buildSynthesisPrompt() и hardcoded промпты
  - Обновлена DI конфигурация (добавлен SynthesizeExpertOpinionsUseCase)
  - Проект успешно собирается
- ✅ Завершена **Фаза 1.3.3 - Рефакторинг ConversationViewModel**
  - Создан GetMessagesWithExpertOpinionsUseCase (23 строки)
  - Удалены все прямые вызовы conversationRepository из ViewModel
  - Обновлен ConversationViewModel для использования GetMessagesWithExpertOpinionsUseCase
  - Удален неиспользуемый импорт ConversationRepository
  - Обновлена DI конфигурация (добавлен GetMessagesWithExpertOpinionsUseCase в DomainLayerKoinModule и ConversationKoin)
  - ConversationViewModel теперь работает только через use cases (0 прямых обращений к repository)
  - Clean Architecture соблюдается на 100%
  - Проект успешно собирается
- ✅ Завершена **Фаза 2.1.1 - Устранение дублирования в MessageSendingServiceImpl**
  - Создан общий метод parseAndCreateMessage() (30 строк)
  - MessageSendingServiceImpl упрощен с 163 до 159 строк
  - Устранено дублирование между sendToYandex() и sendToProxy()
  - Проект успешно собирается
- ✅ Завершена **Фаза 2.1.2 - Централизация парсинга JSON**
  - Заменен прямой вызов Json.decodeFromString в ExecuteChainTwoAgentsUseCase
  - Все парсинги теперь используют ParseAssistantResponseUseCase
  - Добавлена обработка ошибок парсинга через Result<T>
  - Обновлена DI конфигурация для ExecuteChainTwoAgentsUseCase
  - 0 прямых вызовов Json.decodeFromString<AssistantJsonAnswer> в кодебазе
  - Проект успешно собирается
- ✅ Завершена **Фаза 2.1.3 - Централизация системных промптов**
  - SystemPromptBuilder расширен с 172 до 205 строк
  - Добавлен метод buildChainAnalystPrompt() для цепочки агентов
  - ExecuteChainTwoAgentsUseCase упрощен с 95 до 83 строк
  - Удалено ~25 строк хардкоженного промпта
  - 0 хардкоженных промптов в use cases (все централизованы в SystemPromptBuilder)
  - Обновлена DI конфигурация для ExecuteChainTwoAgentsUseCase
  - Проект успешно собирается
