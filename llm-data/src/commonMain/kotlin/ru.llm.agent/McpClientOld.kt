...
public class McpClientOld (private val serverUrl: String = "http://kalimruslan-rt.ru/mcp") {
    ...
    public suspend fun initialize(): Result<InitializeResult> = withContext(Dispatchers.IO) {
        try {
            ...
            getLogger("McpClient").info("Request: ${json.encodeToString(McpRequest.serializer(), request)}")
            ...
            val response = json.decodeFromString<McpToolsListResponse>(httpResponse.bodyAsText())
    ...
    public suspend fun listTools(): Result<List<McpTool>> = withContext(Dispatchers.IO) {
        try {
            val request = McpRequest(
                id = generateId(),
                method = "tools/list"
            ...
            if (response.error != null) {
                ...
                Result.success(result.mcpTools)
            }
        } catch (e: Exception) {
    ...
    public suspend fun callTool(
        ...
        try {
            ...
            if (response.error != null) {
                ...
                val result = Json.decodeFromJsonElement<McpToolsResult>(response.result!!)
